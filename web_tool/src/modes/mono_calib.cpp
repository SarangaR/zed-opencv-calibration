...
#include "monocal/calibration_checker.hpp"
#include "monocal/opencv_calibration.hpp"
#include "web_common.hpp"

namespace mono_calib_mode {

using namespace monocal;
    ...
    while (1) {
      ...
      if (frame_ok) {
        ...
        if (acquisition_completed) {
          ...
          cv::putText(
              text_info,
              "Capture frames with the web buttons (or auto-capture).",
          ...
          // Auto-capture indicator; toggled from the web UI (/ctrl?auto=).
        ...
        // attempts a (throttled) testSample whenever a board is detected —
        // the checker only accepts frames that improve sample diversity, so
        // holding the board still does not flood the database with
        // duplicates.
...
int run_mono_calib(int argc, char** argv, HttpStreamer& streamer) {
  return mono_calib_mode::run(argc, argv, streamer);
}
