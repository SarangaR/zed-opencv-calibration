...
#include "stereocal/calibration_checker.hpp"
#include "stereocal/opencv_calibration.hpp"
#include "web_common.hpp"

namespace stereo_calib_mode {

using namespace stereocal;
...
int run(int argc, char* argv[], HttpStreamer& streamer) {
  ...
  if (!args.use_stored_images) {
    ...
    // Web mode has no keyboard: auto-capture is always on, Stop comes from
    // the launcher as SIGINT (g_stop_requested).
    ...
    while (1) {
      ...
      if (zed_camera.grab() == sl::ERROR_CODE::SUCCESS) {
        ...
        if (acquisition_completed) {
          ...
          cv::putText(
              rendering_image,
              "Capture frames with the web buttons (or auto-capture) when "
          ...
          // Auto-capture indicator, top-right of the bottom panel; toggled
          // from the web UI (/ctrl?auto=).
          const bool auto_on = streamer.autoCaptureEnabled();
          const cv::Rect auto_btn_rect(rendering_image.cols - 320,
                                       display.size[0] + 12, 300, 36);
          cv::rectangle(rendering_image, auto_btn_rect,
                        auto_on ? cv::Scalar(40, 120, 40)
                                : cv::Scalar(60, 60, 60),
                        -1);
          cv::rectangle(rendering_image, auto_btn_rect,
                        auto_on ? info_color : cv::Scalar(160, 160, 160), 2);
          cv::putText(rendering_image,
                      auto_on ? "AUTO CAPTURE: ON" : "AUTO CAPTURE: OFF",
                      cv::Point(auto_btn_rect.x + 14, auto_btn_rect.y + 25),
                      cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255),
                      2);
        }

        streamer.publish(rendering_image);
        ...
        // Manual capture from the web UI's Capture button; auto capture
        // attempts a (throttled) testSample whenever the board is detected
        // in both views — the checker only accepts frames that improve
        // sample diversity.
        const bool manual_capture = streamer.consumeCaptureRequest();
        const double now_s =
            static_cast<double>(cv::getTickCount()) / cv::getTickFrequency();
        const bool auto_attempt = streamer.autoCaptureEnabled() &&
                                  !manual_capture && found_l && found_r &&
...
int run_stereo_calib(int argc, char** argv, HttpStreamer& streamer) {
  return stereo_calib_mode::run(argc, argv, streamer);
}